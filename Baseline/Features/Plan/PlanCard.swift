...
// MARK: - Detail (structural preview + lifecycle; full execution reuse lands next)
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        Text(scheduled.workout.title).font(.system(size: 24, weight: .bold)).foregroundStyle(BaselineColor.textHi)
                        if let goal = scheduled.workout.goal { Text(goal).font(.system(size: 15)).foregroundStyle(BaselineColor.textMid) }
                        if let (label, color) = PlanStatusStyle.chip(plan.status(for: scheduled, today: Date())) {
                            Text(label).font(.system(size: 12, weight: .bold)).tracking(0.6).foregroundStyle(color)
                        }
                        ForEach(scheduled.workout.blocks) { block in
                            if !block.isDefault && !block.name.isEmpty {
                                Text(block.name.uppercased()).font(.system(size: 12, weight: .bold)).tracking(0.6).foregroundStyle(BaselineColor.textFaint).padding(.top, 6)
                            }
                            ForEach(block.exercises) { ex in
                                HStack {
                                    Text(ex.exerciseName).font(.system(size: 16, weight: .semibold)).foregroundStyle(BaselineColor.textHi)
                                    Spacer()
                                    Text("\(ex.prescription.sets.count) sets").font(.system(size: 14)).foregroundStyle(BaselineColor.textFaint)
                                }
                                .padding(.vertical, 10)
