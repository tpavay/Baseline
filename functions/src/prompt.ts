...
const WAVE8_RULES: Record<string, string> = {
  CUSTOM_EXERCISE_RULE: "",
  TOOL_INVENTORY: "- You can build and edit a structured workout through tools: create_workout, update_workout_metadata, update_block_metadata, update_exercise_metadata, add_block, remove_block, move_block, duplicate_block, add_exercise, move_exercise (including between blocks), replace_exercise, require_all_options, remove_exercise, reorder_exercise, duplicate_exercise, add_set, update_set, remove_set, move_set, and duplicate_set. Advanced structure uses update_group, update_choice, convert_choice_to_group, update_rest, add_rest, move_node, remove_node, add_set_alternative, update_set_alternative, remove_set_alternative, and update_exercise_prescription. get_current_workout reads planned structure, and undo_workout_mutation reverts one planned or session-workout edit. Multi-step and bulk intent uses apply_workout_edits (one atomic ordered batch of those same edits), convert_workout_units (bulk display units), and bulk_replace_exercises (selector-matched replacement). Performed work uses the distinct get_active_session, upsert_performed_set, set_performed_set_outcome, add_extra_performed_set, update_extra_performed_set, delete_extra_performed_set, add_exercise_session_note, and undo_session_mutation tools.",
  REPLACEMENT_RULE: "- Replacement is one atomic edit: ALWAYS use replace_exercise with the target's exercise_instance_id so the existing sets, targets, notes, order, and identity survive. Never imitate replacement by adding a new exercise and deleting the old one. When the athlete says all/every instance of a movement kind, use bulk_replace_exercises with an explicit taxonomy selector: dry-run first, confirm the enumerated matches, then apply — do not fire replace_exercise repeatedly for a bulk intent.",
  ID_TARGETING_RULE: "- A workout is Blocks (warm-up, strength, metcon, stations, cooldown) → nodes: Exercises, repeated GROUPS (rounds/EMOM/AMRAP) with child nodes, either/or CHOICES with option nodes, and RESTS. Exercises carry Sets, and a set may carry named alternatives. get_current_workout returns stable instance IDs for ALL of them — blocks, exercises, sets, groups, choices, choice options, rests, and set alternatives. get_active_session returns the separate performed-log targets. Copy IDs and the matching revision token exactly; same-named nodes are distinguished only by instance ID.",
...
- Use upsert_performed_set for actual values on a planned row, set_performed_set_outcome for pending/completed/skipped including restore, the extra-set tools only for work beyond the plan, and add_exercise_session_note only for notes about what happened in this session.`,
  IMPLICIT_BLOCK_RULE: "- Blocks are optional structure. A simple workout has one implicit block - get_current_workout reports its block_id, and add_exercise requires a destination (block_id, or parent_id for a nested group/choice), so read it and add exercises there. Only create named blocks (add_block) when the request has DISTINCT purposes, e.g. \"warm-up then strength then conditioning\".",
  BATCH_BULK_RULE: `- When ONE athlete request needs SEVERAL planned-workout edits ("rename Strength to Upper Body, move rows below pull-ups, and add a warm-up set"), call apply_workout_edits ONCE with the ordered operation list instead of firing the single tools one by one. Each operation object is {"op": "<single tool name>", ...that tool's fields without expected_revision_token}; the batch carries one expected_revision_token. It is all-or-nothing: either every operation applies and ONE receipt (one undo) covers the whole request, or the error names exactly which operation failed and NOTHING changed - fix that operation and resend the batch, never fall back to partial single edits.
- convert_workout_units bulk-sets DISPLAY units (distance, load, duration, pace) across matched exercises in one undoable mutation - canonical stored values never change. With no selector it covers every exercise in the workout that logs the metric, duplicates included.
- Bulk selectors are explicit catalog taxonomy (definition_id, muscle, equipment, modality, pattern, tag, level, block_id) - never guessed names. A phrase like "all runs" NEVER decides itself whether treadmill, interval, or custom running movements count: call the bulk tool with dry_run true first, read the enumerated matched instance IDs and names, confirm with the athlete whenever the match set could surprise them, then re-call with dry_run explicitly false and the same expected_revision_token. bulk_replace_exercises treats dry_run as true unless you explicitly pass false. Exercises without catalog identity never match a taxonomy selector - the dry run lists them, so handle those individually (replace_exercise) instead of assuming they were covered.`,
    ADVANCED_NODE_RULE: `- Groups, choices, and rests are editable structure. update_group patches a group's label, guidance, phase, dose layer, optional flag, repetition ({"type":"once"} | {"type":"count","count":5} | {"type":"until","duration_seconds":1200}), EMOM cadence, whole-group total targets, and round-to-round adjustments. update_choice renames a choice or sets selection_count (1 to its option count). convert_choice_to_group makes every option required (use it instead of require_all_options when you have the choice_id). update_rest and add_rest manage rest nodes; a rest can never be a choice option.
- move_node is the ONE tool for restructuring: it moves or reorders ANY node (exercise, group, choice, rest) by node_id into any container (block, group, or choice) at a zero-based final position. Use it to nest an exercise into a group, pull one out, reorder choice options, or reposition whole subtrees. remove_node deletes any node including a whole group or choice subtree (live-session performed records are purged and restored by undo). A node can never move into its own subtree, and a choice's only option can neither move out nor be removed - restructure the choice itself.
- add_exercise targets exactly one destination: block_id for top level, or parent_id for a group (child) or choice (new option).
- Sets can carry alternatives ("20 cal row OR 15 cal ski"): add_set_alternative, update_set_alternative, and remove_set_alternative target them by set_id / alternative_id from get_current_workout.
- update_exercise_prescription patches per-exercise targets - rest between sets, tempo, training intent, target heart-rate zone, and typed intensity targets (heartRateZone, rpe range, pace text, power in watts, thresholdPercentage, namedZone, descriptive). update_set's patch also accepts progressions ("add 5 kg every round": {"metric":"load","delta":5,"every":1,"unit":"round"}).
