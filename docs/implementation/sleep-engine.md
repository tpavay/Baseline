...
| HealthKit sleep | `HealthService.sleepSummary(nightsAgo:)` (`HealthService.swift`): asleep hours, deep/REM/core hours, efficiency, noon-to-noon window | Discards sample timestamps → no bedtime/wake, no awakenings, no gaps, no timeline data, no per-interval source attribution, no anchored delta sync, no multi-source resolution |
...
| Manual fallback | `SleepCheckInCard` thumbs/stepper → `CheckInAnswers`; mapped to 85/35 in `MorningReadinessScoreView.compute()` (`DailyReadingFlowView.swift`) | Preserved as-is: manual nights are recorded but never presented as measured scores |
