...
- Complex but legible prescriptions remain valid sketch output: the movement skeleton is structured, while EMOM rules, conditionals, nested repeats, multi-phase RPE schemes, and other unsupported details remain as notes.
  The sketch and the parsed document still carry notes at the workout, block, group, and exercise levels, but Baseline shows notes at two levels only - the workout and the exercise - so `WorkoutImportDraftBuilder` folds block and group notes up into the workout's note, each prefixed with the section it described, rather than leaving them where nothing renders them.
  That workout note is the single free-form field the athlete reads and edits, so the document's goal and every folded note join into `Workout.goal` separated by blank lines.
  Imported prose cannot become workout-level `CoachGuidance`, because `Workout` has no such field; per-exercise notes still materialize as that exercise's `CoachGuidance`.
