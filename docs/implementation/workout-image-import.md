...
- Complex but legible prescriptions remain valid sketch output: the movement skeleton is structured, while EMOM rules, conditionals, nested repeats, multi-phase RPE schemes, and other unsupported details remain as notes.
  ...
  That workout note is the single free-form field the athlete reads and edits, so the document's goal and every folded note join into `Workout.goal` separated by blank lines; imported prose never becomes `CoachGuidance`, whose components stay reserved for structured cues.
