...
## What gets stored
**Local-first, with named exceptions.** Training state - sleep nights, plans, sessions, workout logs, HRV readings, heart-rate zone settings - lives in SwiftData on device and is never written to the backend.
Three things do leave it: the onboarding profile stored at `users/{uid}` in Firestore; the conversation transcript plus a derived context summary (readiness, logged sleep, check-in, constraints, active workout) sent to the `conversation` function and on to the model provider, along with whatever an explicit retrieval tool returned; and workout-import source material. Import has two transports and they send different things: the durable multi-image job sends on-device OCR text only, retained server-side for 24 hours, while the single-photo fast path (`streamWorkoutImport`) sends the normalized photo itself alongside that text. Both are relayed to the model provider in memory. The photo is passed through and not retained by Baseline: `functions/` uses no Cloud Storage at all, the fast path writes nothing to Firestore beyond the pre-existing per-user daily counter, Cloud Logging records an image count and never image bytes, and the Langfuse path neither captures request content for that turn nor could export it (traces carry `content_capture` `none`, and the exporter's `sanitizeExportValue` mask is a strict allowlist that redacts any string not on it). No transcript is persisted server-side.
`Baseline/PrivacyInfo.xcprivacy` is the authoritative declaration of what Baseline collects and links to the user - keep it and the App Store privacy answers in step with anything that changes here. Transmitted photos are declared there as `NSPrivacyCollectedDataTypePhotosorVideos`, unlinked and untracked; `BaselineTests/AppStoreValidationTests.swift` fails the build if that entry disappears. Sending pixels rather than recognized text is deliberate: layout is information, an app screenshot is a table, and which number sits in which column is carried by position that OCR text discards.
The **structured state is the source of truth** — training profile, daily context, constraints, daily readiness/plan entries, accepted plan versions, coach guidance, workout logs, athlete notes, skips, substitutions, pain events, and chat *summaries*. Never the raw chat log.
