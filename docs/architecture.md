...
Three things do leave it: the onboarding profile stored at `users/{uid}` in Firestore; the conversation transcript plus a derived context summary (readiness, logged sleep, check-in, constraints, active workout) sent to the `conversation` function and on to the model provider, along with whatever an explicit retrieval tool returned; and on-device OCR text from a workout import, retained server-side for 24 hours. Source photos never leave the device, and no transcript is persisted server-side.
