...
Three things do leave it: the onboarding profile stored at `users/{uid}` in Firestore; the conversation transcript plus a derived context summary (readiness, logged sleep, check-in, constraints, active workout) sent to the `conversation` function and on to the model provider, along with whatever an explicit retrieval tool returned; and workout-import source material. Import has two transports and they send different things: the durable multi-image job sends on-device OCR text, retained server-side for 24 hours, while the single-photo fast path (`streamWorkoutImport`) sends the normalized photo itself alongside that text, and both are relayed to the model provider in memory and never written to Firestore, Cloud Storage, or logs. No transcript is persisted server-side.
`Baseline/PrivacyInfo.xcprivacy` is the authoritative declaration of what Baseline collects and links to the user - keep it and the App Store privacy answers in step with anything that changes here.
