...
Copy Hevy's *interaction density*, not its product. **Exercises are collapsible document rows, not cards** — a name row separated by whitespace/thin rules; collapsed by default, expanding inline into a **logging table** (`# · TIME · DIST · ⋯` — columns generated from the exercise's selected metrics). A 40-exercise workout is 40 rows, not 40 giant cards. **Blocks are lightweight section headers** (Apple-Notes style: a label + hairline rule), *not* cards — organization, not decoration; they let the exercises breathe, and the implicit default block shows nothing at all. Baseline layers its value *on expand*: today's goal · context · coach guidance appear only when an exercise is open (not always). Two reads of the same document: **Planning** (collapsed, scan/edit/reorder) vs **Training** (expanded, optimized for logging). *(Built: light rows, section-header blocks, collapse-by-default, dynamic metric columns. Follow-ons: long-press-drag reorder — needs a List/drag layer; coach-guidance-on-expand — needs the guidance model; per-row thumbnails.)*
