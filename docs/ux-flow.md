...
Copy Hevy's *interaction density*, not its product. **Exercises are collapsible document rows, not cards** — a name row separated by whitespace/thin rules; collapsed by default, expanding inline into a **logging table** (`# · TIME · DIST · ⋯` — columns generated from the exercise's selected metrics). A 40-exercise workout is 40 rows, not 40 giant cards. **Blocks are lightweight section headers** (Apple-Notes style: a label + hairline rule), *not* cards — organization, not decoration; they let the exercises breathe, and the implicit default block shows nothing at all. Baseline layers its value *on expand*: today's goal · context · coach guidance appear only when an exercise is open (not always). *(Built: light rows, section-header blocks, collapse-by-default, dynamic metric columns. Follow-ons: long-press-drag reorder — needs a List/drag layer; coach-guidance-on-expand — needs the guidance model; per-row thumbnails.)*
...
- **Training** (started): each cell pre-fills with the plan value *as its placeholder*; typing records the **actual** (stored on the log, never overwriting the plan). "If you only managed 170×12, that's what gets stored." A **✓ per set row** is the completion gesture — check it and the row grays, the next becomes active; when every set is checked the **exercise auto-completes**. There is no exercise-level "Complete" button and no separate log sheet. Skip lives in the ⋯ menu (*Skip exercise*).

The exercise **⋯ menu** is: *Metrics* (which columns) · *Units* (how each shows) · *Replace* (substitute) · *Move* (block / up / down) · *Duplicate* · *Skip exercise* (training only) · *Delete*. Deferred until their data exists: *Coach Guidance*, *History*, and the **PREVIOUS** column (last session's values, the natural progression cue) — none faked with placeholder data.
