...
- Adding or changing a conversational agent tool is one serialized facade change, and shipping only part of it produces tools the model never uses.
  ...
  Any change to a served schema or system prompt also requires regenerating `functions/src/toolSchemaTokens.json` via `npm --prefix functions run tokens:measure` (needs `ANTHROPIC_API_KEY`); CI's exact `tokens:check` fails on a stale fixture.
