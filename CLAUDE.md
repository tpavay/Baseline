...
- Adding or changing a conversational agent tool is one serialized facade change, and shipping only part of it produces tools the model never uses.
  ...
  Structural node edits (locate, extract, insert, move, remove, typed updates) all go through the one recursive node API in `WorkoutNode.swift` (`withNodeLists`/`mutateNodeLists` and the wrappers on `Workout`); never add a new ad hoc recursive walker.
  Tool `input_schema` must stay inside the documented safe subset in `docs/tool-schema-contract.md` (no top-level `oneOf`/`allOf`/`anyOf`/`not`, no `$ref`, no conditionals, allowlisted keywords only): a construct a provider's request validator rejects 400s every conversation for every user served that toolset (this broke the whole feature in 2026-07).
  Express exclusivity with schema-form `dependencies` plus the description, and rely on `ToolCallMapper`'s deterministic validation; the offline contract lint (`functions/test/toolSchemaContract.test.js`, in `npm test`) is the per-PR guard that fails a bad schema for $0, and the gated live-API preflight (`npm --prefix functions run preflight:providers`, in `provider-live-guard.yml`) is the paid ground-truth backstop. `count_tokens` is not an acceptance check - it returns 200 for a top-level `oneOf`; see `docs/tool-schema-contract.md`.
  Any change to a served schema or system prompt also requires regenerating `functions/src/toolSchemaTokens.json`; contributors without a local Anthropic key use the read-only workflow in `functions/scripts/README.md`, while maintainers with a key may run `npm --prefix functions run tokens:measure`.
  CI's exact `tokens:check` fails on a stale fixture.
...
- **CI:** `.github/workflows/ci.yml` runs on every PR, to any base branch (no `paths:` or `branches:` filter, so no PR can ever be stuck on a required check that never reports): Debug simulator build + full Swift test suite, an unsigned Staging device compile, the Cloud Functions type-check + test suite (whose `npm test` is the offline tool-schema contract lint - the per-PR provider-rejection guard), and (on functions changes) `functions-token-fixture`, which verifies the committed tool-schema token fixture via the **free** `count_tokens` endpoint. Routine CI spends **$0** of real provider tokens. The paid live-API preflight + conversation smoke moved off the per-PR path into the opt-in `.github/workflows/provider-live-guard.yml` (manual dispatch, weekly schedule, or a PR labeled `provider-smoke`) - keyed by the `ANTHROPIC_API_KEY` repository secret and documented in `docs/tool-schema-contract.md`.
